...
namespace TheDotFactory
{
    public partial class MainForm : Form
    {
        ...
        {
            string all = "", numbers = "", letters = "", uppercaseLetters = "", lowercaseLetters = "", symbols = "", cp1251 = "";
            int[] table1251 = { 0x0402, 0x0403, 0x201A, 0x0453, 0x201E,  0x2026, 0x2020, 0x2021, 0x20AC, 0x2030, 0x0409,  0x2039, 0x040A,  0x040C,  0x040B,  0x040F,
            ...
            // generate characters
            for (char character = ' '; character < 256; ++character)
            {
                // add to all
                ...
                {
                    // 1251
                    if (character > 127 && character < 192)
                    {
                        cp1251 += (char)table1251[character-128];
                    }
                    else
                    {
                        cp1251 += (char)(character - 192 + 0x0410);
                    }
            ...
            cbxTextInsert.Items.Add(new ComboBoxItem("Codepage-1251", cp1251));
        ...
        private FontInfo populateFontInfo(Font font)
        {
            ...
            fontInfo.generatedChars = txtInputText.Text;
        ...
        private void generateStringsFromFontInfo(FontInfo fontInfo, ref string resultTextSource, ref string resultTextHeader)
        {
            ...
            resultTextSource += String.Format("// Height = {0} bit" + nl,
                                                   fontInfo.charHeight);
            ...
            // table width
            //********************************
            //resultTextSource += "typedef struct" + nl + "{" + nl + "\tuint16_t Width;" + nl + "\tuint16_t Position;" + nl +"} FONT_INFO;" + nl + nl;
            resultTextSource += "uint16_t fontPositionTable[] = " + nl + "{" + nl;
            int position;
            position = 0;
            // iterate through letters
            for (int charIdx = 0; charIdx < fontInfo.characters.Length; ++charIdx)
            {
                    // output character header
                resultTextSource += String.Format("\t{1},\t\t//\t'{2}',\t{3}" + nl,
                                                        fontInfo.characters[charIdx].width,
                                                        position,
                                                        fontInfo.characters[charIdx].character,
                                                        charIdx
                                                        );
                position += (((fontInfo.characters[charIdx].width+7) / 8))*fontInfo.charHeight;
                    // space between chars
                    //resultTextSource += nl;
            }
            // space out
            ...
            position = 0;
            resultTextSource += "uint16_t fontWidthTable[] = " + nl + "{" + nl;
            // iterate through letters
            for (int charIdx = 0; charIdx < fontInfo.characters.Length; ++charIdx)
            {
                // output character header
                resultTextSource += String.Format("\t{0},\t\t//\t'{2}',\t{3}" + nl,
                                                        fontInfo.characters[charIdx].width,
                                                        position,
                                                        fontInfo.characters[charIdx].character,
                                                        charIdx
